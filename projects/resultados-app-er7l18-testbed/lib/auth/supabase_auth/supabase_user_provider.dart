...
class ResultadosAppSupabaseUser extends BaseAuthUser {
  ResultadosAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> resultadosAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ResultadosAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
